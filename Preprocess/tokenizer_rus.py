...
NORMALIZE_RE = re.compile(r"([a-zA-Zа-яА-ЯёЁ])\1\1+")
ALPHA_DIGITS_RE = re.compile(r"[0-9_]+")
...
UTF_CHARS = r'a-zа-яё0-9_\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff'
TAG_EXP = r'(^|[^0-9A-ZА-ЯЁ&/]+)(#|\uff03)([0-9A-ZА-ЯЁ_]*[A-ZА-ЯЁ_]+[%s]*)' % UTF_CHARS
...
class CrazyTokenizer(object):
    ...
    def __init__(self, lowercase=True, keepcaps=False, normalize=3,
        ...
        #self._nlp = English()
        self._nlp = Russian()
        russian_tokenizer = RussianTokenizer(self._nlp, MERGE_PATTERNS + SYNTAGRUS_RARE_CASES)
